...
Generated pass templates and detailed integration recipes follow the validated [Microsoft Olive 0.13.0 schema](https://microsoft.github.io/Olive/0.13.0/schema.json): `input_model` contains required `type` and `config` fields, so the model ID is at `input_model.config.model_path`; Hugging Face identifiers use `input_model.type: "HfModel"`; pass entries use arrays of `{ "type": ..., "config": ... }`; and data components use `{ "type": ..., "params": ... }`.
